...
class AdminPanel:
    ...
    def endpoints(lang: str = "zh-CN") -> list[dict[str, Any]]:
        ...
        return [
            ...
            endpoint("GET", "/api/seed/{map_id}", "client", descriptions["api_seed"]),
            endpoint("GET", "/api/model", "client", descriptions["api_model"]),
            endpoint("POST", "/api/down", "client", descriptions["api_down"]),
            endpoint("GET", "/api/legal/terms", "client", descriptions["api_terms"]),
            endpoint("GET", "/api/legal/privacy", "client", descriptions["api_privacy"]),
            endpoint("GET", "/api/legal/eula", "client", descriptions["api_eula"]),
...
def endpoint_descriptions(lang: str) -> dict[str, str]:
    zh = {
        ...
        "api_register": "Registers a user with uid, nickname, email, password_md5, and initializes player state.",
        "api_login": "Logs in a user by uid and password_md5, returns a session token, and writes the session cookie.",
        ...
        "api_seed": "Returns a map seed from seed/map/<map_id>.txt for Perlin terrain generation.",
        "api_model": "Returns the startup-scanned model manifest from the model directory.",
        "api_down": "Downloads a model file from model/ when the request body includes a model path.",
        ...
        "api_eula": "Returns End User License Agreement Markdown; supports the lang parameter.",
